...
var dronePlugin *feishu_robot_oss_plugin.Plugin

// GlobalBeforeAction
// do command Action before flag global.
func GlobalBeforeAction(c *cli.Context) error {
	isDebug := feishu_robot_oss_plugin.IsBuildDebugOpen(c)
	if isDebug {
		drone_log.OpenDebug()
	}
	...
	p, err := feishu_robot_oss_plugin.BindCliFlag(c, cliVersion, pkgJson.GetPackageJsonName(), drone)
	if err != nil {
		return err
	}
	dronePlugin = p
